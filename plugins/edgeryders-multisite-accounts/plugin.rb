...
after_initialize do
  ...
  module ::EdgerydersMultisiteAccounts
    ...
    def self.create_sso_provider_account(args = {})
      ...
      master_api_key = Rails.application.secrets.sso_provider[:api_key]
      client = DiscourseApi::Client.new("#{protocol}://#{hostname}", master_api_key, "system")
      client.create_user(
          name: args[:username],
          email: args[:email],
    ...
    def self.create_user_api_key(hostname, username, api_key)
      require 'net/http'
      require 'json'
      uri = URI.parse "#{protocol}://#{hostname}/admin/api/keys"
      ...
      request.initialize_http_header(
          {
              ...
              "Api-Key" => "#{api_key}",
              "Api-Username" => "system", 'Cache-Control' => "no-cache"
          })
